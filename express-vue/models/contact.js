...
const getListing = async (req, select = {}, where = {}) => {
    try {
        ...
        let listing = contact.find(where, select, { skip: offset })
            .sort(orderBy)
            .limit(limit)
...
const getCounts = async (where = {}) => {
    try {
        let record = await contact.countDocuments(where);
        return record;
    }
    catch (error) {
        console.log(error)
...
const remove = async (id) => {
    try {
        ...
        if (getRecord) {
            let record = await contact.deleteOne({ '_id': id }); // Deleting a record from the 'post' collection that matches the specified '_id'
...
const getById = async (id, select = []) => {
    try {
        let record = await contact.findById(id, select); // Finding a record in the 'post' collection by its '_id'
