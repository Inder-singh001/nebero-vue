...
const getListing = async (req, select = {}, where = {}) => {
    try {
        ...
        let listing = note.find(where, select, { skip: offset })
            .sort(orderBy)
            .limit(limit)
...
const getCounts = async (where = {}) => {
    try {
        let record = await note.countDocuments(where);
        return record;
    }
    catch (error) {
        console.log(error)
...
const remove = async (id) => {
    try {
        ...
        if (getRecord) {
            let record = await note.deleteOne({ '_id': id }); // Deleting a record from the 'post' collection that matches the specified '_id'
...
const getById = async (id, select = []) => {
    try {
        let record = await note.findById(id, select); // Finding a record in the 'post' collection by its '_id'
